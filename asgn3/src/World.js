...
let canvas, gl, cube, camera;
let aPosition, aUV;
let uModelMatrix, uViewMatrix, uProjectionMatrix, uWhichTexture, uBaseColor;
...
const FSHADER_SOURCE = `
  ...
  void main() {
    if (uWhichTexture == -1) {
      ...
      gl_FragColor = texture2D(uSampler3, vUV);
    } else {
...
function drawCube(matrix, textureNum) {
  cube.render(matrix, textureNum, aPosition, aUV, uModelMatrix, uWhichTexture);
}

function drawColorCube(matrix, color) {
  gl.uniform4fv(uBaseColor, color);
  cube.render(matrix, -1, aPosition, aUV, uModelMatrix, uWhichTexture);
}
